...
var pmInitPattern = hooks.MakeFunctionPattern("PM_Init", map[string]string{"Windows": "PM_Init"}, map[string]hooks.SearchPattern{
	common.HL8684: hooks.MustMakePattern("55 8B EC E8 ?? ?? ?? ?? 8B 55 08 33 C0 56 8D 8A ?? ?? ?? ?? 8B B0 ?? ?? ?? ?? 83 C0 0C 89 71 FC 8B B0 ?? ?? ?? ?? 89 31"),
	common.OF8684: hooks.MustMakePattern("8B 44 24 04 A3 ?? ?? ?? ?? E8 ?? ?? ?? ?? E8 ?? ?? ?? ?? C7 05"),
})
var pmPlayerMovePattern = hooks.MakeFunctionPattern("PM_PlayerMove", map[string]string{"Windows": "PM_PlayerMove"}, map[string]hooks.SearchPattern{
	common.HL8684:     hooks.MustMakePattern("A1 ?? ?? ?? ?? 8B 4C 24 04 55 57 33 FF 89 48 04 E8 ?? ?? ?? ?? 8B 15 ?? ?? ?? ?? 33 C9 89 BA 8C 54 04 00 A1 ?? ?? ?? ?? 8A 88 5A 54 04 00 89"),
	common.BigLolly:   hooks.MustMakePattern("55 8B EC 83 EC 0C C7 45 FC 00 00 00 00 A1 ?? ?? ?? ?? 8B 4D 08 89 48 04 E8 ?? ?? ?? ?? 8B 15 ?? ?? ?? ?? C7 82 8C 54 04 00 00 00 00 00 A1"),
	common.TWHLTower2: hooks.MustMakePattern("55 8B EC 51 A1 ?? ?? ?? ?? 8B 4D 08 53 56 57 33 FF 89 7D FC 89 48 04 E8 D8 FC FF FF A1 ?? ?? ?? ?? 89 B8 8C 54 04 00 A1 ?? ?? ?? ?? 0F B6 88 5A 54 04 00"),
})
var csoundEntActiveListPattern = hooks.MakeFunctionPattern("CSoundEnt::ActiveList", map[string]string{common.WindowsHLDLL: "CSoundEnt::ActiveList"}, map[string]hooks.SearchPattern{
	common.HL8684: hooks.MustMakePattern("A1 ?? ?? ?? ?? 85 C0 75 04 83 C8 FF C3 8B 40 58 C3"),
	common.OF8684: hooks.MustMakePattern("A1 ?? ?? ?? ?? 85 C0 75 04 83 C8 FF C3 8B 40 64 C3"),
	common.HLWON:  hooks.MustMakePattern("A1 ?? ?? ?? ?? 85 C0 75 04 83 C8 FF C3 8B 40 24 C3"),
})
var csoundEntSoundPointerForIndexPattern = hooks.MakeFunctionPattern("CSoundEnt::SoundPointerForIndex", map[string]string{common.WindowsHLDLL: "CSoundEnt::SoundPointerForIndex"}, map[string]hooks.SearchPattern{
	common.HL8684: hooks.MustMakePattern("8B 0D ?? ?? ?? ?? 85 C9 75 03 33 C0 C3 8B 44 24 04 83 F8 3F 7E 13 68 ?? ?? ?? ?? 6A 01 FF 15 ?? ?? ?? ??"),
})
var cbaseMonsterChangeSchedulePattern = hooks.MakeFunctionPattern("CBaseMonster::ChangeSchedule", map[string]string{common.WindowsHLDLL: "CBaseMonster::ChangeSchedule"}, map[string]hooks.SearchPattern{
	// Search for COND_HEAR_SOUND
	common.HL8684: hooks.MustMakePattern("8B 44 24 04 33 D2 89 81 78 01 00 00 89 91 7C 01 00 00 89 91 74 01 00 00 89 91 F0 00 00 00 89 91 68 02 00 00"),
	common.OF8684: hooks.MustMakePattern("8B 81 84 01 00 00 33 D2 3B C2 56 74 55 8B 00 3B C2 74 4F 8B B1 88 01 00 00 57 8B 3C F0"),
	common.HLWON:  hooks.MustMakePattern("8B 44 24 04 33 D2 89 81 44 01 00 00 89 91 48 01 00 00 89 91 40 01 00 00 89 91 BC 00 00 00 89 91 34 02 00 00"),
	common.OFWON:  hooks.MustMakePattern("8B 81 48 01 00 00 33 D2 3B C2 56 74 55 8B 00 3B C2 74 4F 8B B1 4C 01 00 00 57 8B 3C F0"),
})
var cbaseMonsterRouteNewPattern = hooks.MakeFunctionPattern("CBaseMonster::RouteNew", map[string]string{
	common.WindowsHLDLL: "CBaseMonster::RouteNew",
	...
	common.HL8684: hooks.MustMakePattern("33 C0 89 81 ?? ?? ?? ?? 89 81 ?? ?? ?? ?? C3 90 8B 81 ?? ?? ?? ?? C1 E0 04"),
})
var cbaseMonsterPBestSoundPattern = hooks.MakeFunctionPattern("CBaseMonster::PBestSound", map[string]string{
	common.WindowsHLDLL: "CBaseMonster::PBestSound",
}, map[string]hooks.SearchPattern{
	// Search for "ERROR! monster %s has no audible sounds!"
	common.HL8684: hooks.MustMakePattern("83 EC 10 53 8B D9 55 57 8B BB 1C 02 00 00 83 CD FF 83 FF FF C7 44 24 0C 00 00 00 46 75 2D"),
	common.OF8684: hooks.MustMakePattern("83 EC 10 53 8B D9 55 57 8B BB 28 02 00 00 83 CD FF 83 FF FF C7 44 24 0C 00 00 00 46 75 2D"),
	common.HLWON:  hooks.MustMakePattern("83 EC 10 53 8B D9 55 57 8B BB E8 01 00 00 83 CD FF 83 FF FF C7 44 24 0C 00 00 00 46 75 2D"),
	common.OFWON:  hooks.MustMakePattern("83 EC 10 53 8B D9 55 57 8B BB EC 01 00 00 83 CD FF 83 FF FF C7 44 24 0C 00 00 00 46 75 2D"),
})
var worldGraphPattern = hooks.MakeFunctionPattern("WorldGraph", map[string]string{
	// Not actually a function
	common.WindowsHLDLL: "WorldGraph",
}, nil)
var cgraphInitGraphPattern = hooks.MakeFunctionPattern("CGraph::InitGraph", map[string]string{
	common.WindowsHLDLL: "CGraph::InitGraph",
}, map[string]hooks.SearchPattern{
	// Search for "Couldn't malloc %d nodes!" to find CGraph::AllocNodes
	// Then find cross reference from CWorld::Precache
	common.HL8684: hooks.MustMakePattern("56 8B F1 57 33 FF 8B 46 10 89 3E 3B C7 89 7E 04 89 7E 08 74 0C 50 E8 ?? ?? ?? ?? 83 C4 04 89 7E 10 8B 46 0C"),
})
