...
typedef struct {
	...
	int num_hull_models;
	struct model_s *current_worldmod;
	int current_hullnum;

	qboolean in_draw_brush;
} HullInfo;


static HullInfo hullInfo;

	void hullLazyInit(model_t *(*Mod_FindName)(int, const char *), int hullnum)
	{
		if (hullInfo.current_worldmod && !hullInfo.hull_models) {
			hullInfo.hull_models = (hull_model_t *)malloc(sizeof(hull_model_t) * hullInfo.current_worldmod->numsubmodels);
			hullInfo.num_hull_models = hullInfo.current_worldmod->numsubmodels;

			// initialize list
			for (int i = 0; i < hullInfo.current_worldmod->numsubmodels; i++)
			{
				hullnode_t *poly = &hullInfo.hull_models[i].polys;
				poly->next = poly;
				poly->prev = poly;
			}

			hullInfo.current_hullnum = 0;
		}

		if (hullInfo.hull_models && hullInfo.num_hull_models && hullnum != hullInfo.current_hullnum) {
			hullInfo.current_hullnum = hullnum;
			make_polygons_for_hull(Mod_FindName, hullnum, hullInfo.current_worldmod, hullInfo.hull_models, hullInfo.num_hull_models);
		}
	}

	void hullDrawWorldHull(float alpha)
	{
		if (!hullInfo.hull_models || !hullInfo.num_hull_models)
			return;

		hull_model_t	*hull = &hullInfo.hull_models[0];
		winding_t		*poly;
	...
	void hullClean()
	{
		if (hullInfo.hull_models) {
			// release ploygons
			for(int i = 0; i < hullInfo.num_hull_models; i++)
			{
				hull_model_t *model = &hullInfo.hull_models[i];
				free_hull_polys( &model->polys );
			}
			free(hullInfo.hull_models);
			hullInfo.hull_models = NULL;
		}

		hullInfo.num_hull_models = 0;
		hullInfo.current_hullnum = 0;
	}


void hullResetWorldModel(model_t *model) 
{
	hullInfo.current_worldmod = model;
}